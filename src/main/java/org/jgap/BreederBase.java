...
implements IBreeder {
	 ...
	 * @since 3.2
	 */
	protected Population applyNaturalSelectors(final Configuration a_config,
			final Population a_pop, final boolean a_processBeforeGeneticOperators) {
		/**@todo optionally use working pool*/
		final boolean monitorActive = a_config.getMonitor() != null;
		try {
			final int selectorSize = a_config.getNaturalSelectorsSize(
			...
			if (selectorSize > 0) {
				...
				final Population new_population = new Population(a_config, population_size);
	 ...
	 * @since 3.2
	 */
	protected void applyGeneticOperators(final Configuration a_config, final Population a_pop) {

...
implements Configurable, Serializable, ICloneable, Comparable {
	 ...
	 * @since 1.0
	 */
	public Configuration(final String a_id, final String a_name) {
		...
		// --------------------------------------------------------------------
		final String clazz = System.getProperty(PROPERTY_JGAPFACTORY_CLASS);
	 ...
	 * @since 2.3
	 */
	public Configuration(final String a_configFileName, final boolean a_ignore)
	...
	public static void reset(final String a_id) {
		final String threadKey = getThreadKey(Thread.currentThread(), a_id);
	...
	protected void checkProperty(final String a_propname, final Object a_obj,
		...
		final String instanceHash = System.getProperty(threadKey + a_propname, null);
		final String key = makeKey(a_obj);
	...
	protected String makeKey(final Object a_obj) {
		final String key = String.valueOf(a_obj.hashCode())
	...
	public synchronized void verifyStateIsValid()
		...
		// -------------------------------------------------------------------
		final Gene[] sampleGenes = m_sampleChromosome.getGenes();
		for (int i = 0; i < sampleGenes.length; i++) {
			final Gene sampleCopy = sampleGenes[i].newGene();
	 ...
	 * @since 1.1
	 */
	public void addNaturalSelector(final NaturalSelector a_selector,
	 ...
	 * @author Klaus Meffert
	 */
	public void setMinimumPopSizePercent(final int a_minimumSizeGuaranteedPercent) {
	 ...
	 * @since 2.0
	 */
	public void setFitnessEvaluator(final FitnessEvaluator a_fitnessEvaluator) {
	 ...
	 * @since 2.1
	 */
	public void setPreservFittestIndividual(final boolean a_preserveFittest) {
	...
	public String toString() {
		...
		if (getGeneticOperators() == null) {
			...
			final int gensize = getGeneticOperators().size();
			if (gensize < 1) {
				result += S_NONE;
			}
			else {
	 ...
	 * @since 2.4
	 */
	public void setKeepPopulationSizeConstant(final boolean a_keepPopSizeConstant) {
		m_keepPopulationSizeConstant = a_keepPopSizeConstant;
	}

	public void setSelectFromPrevGen(final double a_percentage) {
	 ...
	 * @since 3.01
	 */
	protected static String getThreadKey(final Thread current, final String a_id) {
	 ...
	 * @since 3.01
	 */
	public void setJGAPFactory(final IJGAPFactory a_factory) {
		m_factory = a_factory;
	}
	 ...
	 * @since 3.2
	 */
	public void setBreeder(final IBreeder a_breeder) {
	 ...
	 * @since 3.2.2
	 */
	public void setAlwaysCaculateFitness(final boolean a_alwaysCalculate) {
	 ...
	 * @since 3.01
	 */
	private void readObject(final ObjectInputStream a_inputStream)
	 ...
	 * @since 3.2
	 */
	protected void setId(final String a_id) {
		m_id = a_id;
	}
	 ...
	 * @since 3.2
	 */
	public Configuration newInstance(final String a_id, final String a_name) {
		try {
			final Configuration result = new Configuration(m_name);
	 ...
	 * @since 3.2
	 */
	protected Object doClone(final Object a_objToClone) throws Exception {
		if (a_objToClone != null) {
			final ICloneHandler handler = getJGAPFactory().getCloneHandlerFor(
					a_objToClone, null);
			if (handler != null) {
	...
	public int compareTo(final Object a_other) {
		if (a_other == null) {
			...
			final Configuration other = (Configuration) a_other;
			try {
				...
				.append(m_preserveFittestIndividual,
						...
						.append(m_keepPopulationSizeConstant,
								...
								.append(m_minPercentageSizePopulation,
										...
										.toComparison();
			} catch (final ClassCastException cex) {
	 ...
	 * @since 3.5
	 */
	public void setUniqueKeysActive(final boolean a_active) {

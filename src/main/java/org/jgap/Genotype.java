 ...
 * Genotype is evolved, all of its Chromosomes are also evolved. A Genotype may
 * be constructed normally via constructor, or the static
...
public class Genotype implements Serializable, Runnable {
	...
	/**
	 * Constructs a new Genotype instance with the given array of Chromosomes
	 * and the given active Configuration instance. Note that the Configuration
	 * object must be in a valid state when this method is invoked, or a
	 * InvalidConfigurationException will be thrown.
	 * 
	 * @param a_configuration
	 *            the Configuration object to use
	 * @param a_initialChromosomes
	 *            the Chromosome population to be managed by this Genotype
	 *            instance
	 ...
	 */
	@Deprecated
	public Genotype(final Configuration a_configuration,
			final IChromosome[] a_initialChromosomes)
					throws InvalidConfigurationException {
		this(a_configuration, new Population(a_configuration,
				a_initialChromosomes));
	}
	 ...
	 */
	public Genotype(final Configuration a_configuration,
			final Population a_population) throws InvalidConfigurationException {
	...
	public synchronized IChromosome[] getChromosomes() {
		final Iterator it = getPopulation().iterator();
		final IChromosome[] result = new Chromosome[getPopulation().size()];
		int i = 0;
		while (it.hasNext()) {
	 ...
	 * 
	 * @param a_startIndex
	 *            the index to start the determination with
	 * @param a_endIndex
	 *            the index to end the determination with
	 ...
	 */
	public synchronized IChromosome getFittestChromosome(
			final int a_startIndex, final int a_endIndex) {
		return getPopulation().determineFittestChromosome(a_startIndex,
				a_endIndex);
	}
	 ...
	 * fitness values).
	 * 
	 * @param a_numberOfChromosomes
	 *            the number of chromosomes desired
	 ...
	 * @since 2.4
	 */
	public synchronized List getFittestChromosomes(
			final int a_numberOfChromosomes) {
		return getPopulation().determineFittestChromosomes(
				a_numberOfChromosomes);
	}
	 ...
	 * chromosomes will be included in the next generation population. Note that
	 * the population size not always remains constant (dependent on the
	 * NaturalSelectors used!). To consecutively call this method, use
	 * evolve(int)!!!
	 * 
	 * @author Neil Rotstan
	 * @author Klaus Meffert
	...
	public synchronized void evolve() {
		final IBreeder breeder = getConfiguration().getBreeder();
		final Population newPop = breeder.evolve(getPopulation(),
				getConfiguration());
		setPopulation(newPop);
	}

	/**
	 * Evolves this Genotype the specified number of times. This is equivalent
	 * to invoking the standard evolve() method the given number of times in a
	 * row.
	 * 
	 * @param a_numberOfEvolutions
	 *            the number of times to evolve this Genotype before returning
	 * 
	 * @author Klaus Meffert
	 * @since 1.1
	 */
	public void evolve(final int a_numberOfEvolutions) {
	 ...
	 * 
	 * @param a_monitor
	 *            the monitor used to decide when to stop evolution
	 * 
	 * @return messages of the registered evolution monitor. May indicate why
	 *         the evolution was asked to be stopped. May be empty, depending on
	 *         the implementation of the used monitor
	 * 
	 * @author Klaus Meffert
	 * @since 3.4.4
	 */
	public List<String> evolve(final IEvolutionMonitor a_monitor) {
		a_monitor.start(getConfiguration());
		getConfiguration().setMonitor(a_monitor);
		final List<String> messages = new Vector();
		do {
			evolve();
			final boolean goon = a_monitor.nextCycle(getPopulation(), messages);
			if (!goon) {
				break;
			}
		} while (true);
	...
	/**
	 * @return string representation of this Genotype instance, useful for
	 *         display or debug purposes
	 * 
	 * @author Neil Rotstan
	 * @since 1.0
	 */
	...
	public String toString() {
		...
		for (int i = 0; i < getPopulation().size(); i++) {
			...
			buffer.append(" [");
			buffer.append(getPopulation().getChromosome(i)
	 ...
	 * of Chromosomes will be created according to the setup of the sample
	 ...
	 *             if the given Configuration object is null
	 * @throws InvalidConfigurationException
	 *             if the given Configuration instance is not in a valid state
	 * 
	 * @author Neil Rotstan
	 * @author Klaus Meffert
	 * @since 2.3
	...
	public static Genotype randomInitialGenotype(
		...
		// us.
		// ------------------------------------------------------------------
		final int populationSize = a_configuration.getPopulationSize();
		final Population pop = new Population(a_configuration, populationSize);
		// Do randomized initialization.
		// -----------------------------
		final Genotype result = new Genotype(a_configuration, pop);
	 ...
	 * 
	 * @param a_num
	 *            the number of chromosomes to add
	 * @throws InvalidConfigurationException
	 * 
	 * @author Klaus Meffert
	 * @since 3.2
	...
	public void fillPopulation(final int a_num)
			throws InvalidConfigurationException {
		final IChromosome sampleChrom = getConfiguration()
				.getSampleChromosome();
		final Class sampleClass = sampleChrom.getClass();
		final IInitializer chromIniter = getConfiguration().getJGAPFactory()
				.getInitializerFor(sampleChrom, sampleClass);
		if (chromIniter == null) {
			throw new InvalidConfigurationException(
					"No initializer found for class " + sampleClass);
		}
		try {
			for (int i = 0; i < a_num; i++) {
				getPopulation().addChromosome(
						(IChromosome) chromIniter.perform(sampleChrom,
								sampleClass, null));
			}
	...
	public boolean equals(final Object a_other) {
		try {
			...
			final Genotype otherGenotype = (Genotype) a_other;
	 ...
	 * 
	 * @param a_processBeforeGeneticOperators
	 *            true apply NaturalSelectors applicable before
	 *            GeneticOperators, false: apply the ones applicable after
	 *            GeneticOperators
	 * 
	 * @author Klaus Meffert
	 * @since 2.0
	...
	protected void applyNaturalSelectors(
			final boolean a_processBeforeGeneticOperators) {
		...
		try {
			...
			// -------------------------------------------------------------
			final int selectorSize = m_activeConfiguration
					.getNaturalSelectorsSize(a_processBeforeGeneticOperators);
			if (selectorSize > 0) {
				int population_size = m_activeConfiguration.getPopulationSize();
				if (a_processBeforeGeneticOperators) {
					// Only select part of the previous population into this
					// generation.
					// -----------------------------------------------------------------
				...
				for (int i = 0; i < selectorSize; i++) {
					...
					if (i == selectorSize - 1 && i > 0) {
						// Ensure the last NaturalSelector adds the remaining
						// Chromosomes.
						// ---------------------------------------------------------------
						single_selection_size = population_size
								- getPopulation().size();
					} else {
	...
	public void applyGeneticOperators() {
		final List geneticOperators = m_activeConfiguration
				.getGeneticOperators();
		final Iterator operatorIterator = geneticOperators.iterator();
		while (operatorIterator.hasNext()) {
			final GeneticOperator operator = (GeneticOperator) operatorIterator
					.next();
			applyGeneticOperator(operator, getPopulation(), getPopulation()
					.getChromosomes());
	 ...
	 * 
	 * Step Result ---- ------ 1 31*0 + hashcode_0 = y(1) 2 31*y(1) + hashcode_1
	 * = y(2) 3 31*y(2) + hashcode_2 = y(3) n 31*y(n-1) + hashcode_n-1 = y(n)
	 * 
	...
	public int hashCode() {
		int i;
		final int size = getPopulation().size();
		IChromosome s;
		int twopower = 1;
		// For empty genotype we want a special value different from other
		// hashcode
		// implementations.
		// ------------------------------------------------------------------------
	...
	protected void setPopulation(final Population a_pop) {
		m_population = a_pop;
	}
	 ...
	 * Override this method if you want to ensure that a_chromosomes is not part
	 * of a_population resp. if you want to use a different list.
	 * 
	 * @param a_operator
	 *            the GeneticOperator to call
	 * @param a_population
	 *            the population to use
	 * @param a_chromosomes
	 *            the list of Chromosome objects to return
	 * 
	 * @author Klaus Meffert
	 * @since 2.4
	 */
	protected void applyGeneticOperator(final GeneticOperator a_operator,
			final Population a_population, final List a_chromosomes) {
	 ...
	 * 
	 * @param a_pop
	 *            the population to keep constant in size
	 * @param a_maxSize
	 *            the maximum size allowed for the population
	 * 
	 * @author Klaus Meffert
	 * @since 2.5
	 */
	...
	/**
	 * If used in a Thread: runs the evolution forever. You have to implement a
	 * listener to stop computation sometime. See examples.simpleBooleanThreaded
	 * for a possible implementation of such a listener.
	 * 
	 * @author Klaus Meffert
	 * @since 3.01
	 */
	 ...
	 * @author Klaus Meffert
	 */
	public List<IEvolveJob> getEvolves(final IPopulationSplitter a_splitter)
			throws Exception {
		// We return a list of IEvolveJob instances.
		// -----------------------------------------
		final List result = new Vector();
		final Population[] pops = a_splitter.split(getPopulation());
		// Feed the population chunks into different evolve jobs.
		// ------------------------------------------------------
		for (int i = 0; i < pops.length; i++) {
			final Configuration newConf = (Configuration) getConfiguration()
					.clone();
			// Adjust population size.
			// -----------------------
			/* @todo impl. */
			final EvolveData data = new EvolveData(newConf);
			if (pops[i] == null) {
			...
			final IEvolveJob evolver = new EvolveJob(data);
			result.add(evolver);
		}
		getPopulation().clear();
		return result;
	...
	public void mergeResults(final IPopulationMerger a_merger,
			final EvolveResult[] a_results) throws Exception {
		final int size = a_results.length;
		final Population target = new Population(getConfiguration());
		for (int i = 0; i < size; i++) {
			final EvolveResult singleResult = a_results[i];
			if (singleResult == null) {
				throw new IllegalStateException("Single result is null!");
			}
			final Population pop = singleResult.getPopulation();
			/** @todo use/enhance IPopulationMerger */
			// a_merger.mergePopulations()
			final List goodOnes = pop.determineFittestChromosomes(3);
			for (int j = 0; j < goodOnes.size(); j++) {
				final IChromosome goodOne = (IChromosome) goodOnes.get(j);
	...
	/**
	 * Use an evolution monitor, only to be used when running Genotype as a
	 * thread. Otherwise use method evolve(IEvolutionMonitor)
	 * 
	 * @param a_useMonitor
	 *            true: use evolution monitor, set it via setMonitor
	 * 
	 * @author Klaus Meffert
	 * @since 3.6
	 */
	public void setUseMonitor(final boolean a_useMonitor) {
	 ...
	 * 
	 * @param a_monitor
	 *            the IEvolutionMonitor to use
	 * 
	 * @author Klaus Meffert
	 * @since 3.6
	 */
	public void setMonitor(final IEvolutionMonitor a_monitor) {
		m_monitor = a_monitor;
	}

	public void finish() {
		this.getConfiguration().getBreeder().finish();
	}
}

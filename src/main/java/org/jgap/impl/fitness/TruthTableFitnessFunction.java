...
extends FitnessFunction {
	 ...
	 * @since 3.01
	 */
	public TruthTableFitnessFunction(final Configuration a_conf) {
		m_conf = a_conf;
	}
	 ...
	 * @since 3.1
	 */
	public TruthTableFitnessFunction(final Configuration a_conf, final Map a_truthTable) {
		this(a_conf);
	...
	public double calcFitness(final Map a_actualInputOutput) {
		...
		double delta;
		double deltaAbs;
		final Set keySet = getTruthTable().keySet();
		...
		while (keys.hasNext()) {
			...
			outputValueWanted = ( (Double) getTruthTable().get(inputValueWanted)).
					doubleValue();
			final Double output = (Double) a_actualInputOutput.get(inputValueWanted);
			if (output != null) {

...
extends BreederBase {
	...
	private void fillPopulationRandomlyToOriginalSize(final Configuration a_conf,
	 ...
	 */
	protected void keepPopSizeConstant(final Population a_pop, final Configuration a_conf) {
	...
	protected IChromosome reAddFittest(final Population a_pop, final IChromosome a_fittest) {
	...
	protected void updateChromosomes(final Population a_pop, final Configuration a_conf) {
	...
	private Population removeChromosomesWithoutFitnessValue(final Population a_from_pop,
			final Configuration a_config)
					throws InvalidConfigurationException {
		final Population to_pop = new Population(a_config);
		IChromosome selectedChromosome;
	...
	public Population bulkFunctionEvaluation(final Configuration a_conf,
		...
		if (a_bulkFunction != null) {
			...
			final Population popForBulkFunction = removeEvaluatedChromosomes(a_pop, a_conf);
			if (!m_allChromosomesSoFar.isEmpty()) {
				final Iterator it = popForBulkFunction.getChromosomes().iterator();
				//remove dublicates
				while (it.hasNext()) {
